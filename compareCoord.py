...
filename cvChassis cvBoard dnnChassis dnnBoard
"""
...
ap.add_argument("-t", "--input_txt", required=True,
                help="Input dir of TXT file of coordinates")

ap.add_argument("-o", "--output_dir", required=True,
                help="Output dir of txt file of distances")
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        for i in f.readlines():
            ...
            img, xCDNN, yCDNN = customModel(img)

            cv2.rectangle(img, (xminChassis, ymaxChassis),
                          (xmaxChassis, yminChassis), (0, 255, 0), 2)
            cv2.rectangle(img, (xminBoard, ymaxBoard),
                          (xmaxBoard, yminBoard), (0, 255, 0), 2)
            cv2.circle(img, (xCenterChassis, yCenterChassis),
                       centroid_radi, (0, 255, 0), -1)
            cv2.circle(img, (xCenterBoard, yCenterBoard),
                       centroid_radi, (0, 255, 0), -1)
            ...
            try:
                ...
                cvChassis = calculateDistance(
                    chassisCentroid[0], chassisCentroid[1], xCenterChassis, yCenterChassis)
            except: pass
            ...
            try:
                ...
                cv2.circle(img, chassisBoard,
                           centroid_radi, (0, 0, 255), -1)
                cvBoard = calculateDistance(chassisBoard[0], chassisBoard[1], xCenterBoard, yCenterBoard)
            except: pass
            ...
            for c in range(len(xCDNN)):
                if c == 0:
                    dnnChassis = calculateDistance(xCDNN[c], yCDNN[c], xCenterChassis, yCenterChassis)
                elif c == 1:
                    dnnBoard = calculateDistance(xCDNN[c], yCDNN[c], xCenterBoard, yCenterBoard)

            try: dnnBoard
            except NameError: dnnBoard = None

            filename = i.split()[0]
            final = " ".join([str(filename), str(cvChassis), str(cvBoard), str(dnnChassis), str(dnnBoard), '\n'])
            print(final)
            output_file.write(final)
            #     cvDistance = None
            # # DNN Distance from actual
            # try:
            #     dnnDistance = calculateDistance(
            #             xCenterDNN, yCenterDNN, xCenter, yCenter)
            # except:
            #     dnnDistance = None

            # final = str(filename) + " " + str(cvDistance) + \
                        # " " + str(dnnDistance) + "\n"
            # f.write(final)
